...
const AdminPortal: React.FC<AdminPortalProps> = ({ onRouteToPrinter }) => {
  ...
  const itemsPerPage = 50; // Server-side pagination now handles this
          ...
          <div>
            ...
            <div className="tab-bar mb-4">
              {([
                ...
                { key: 'ready',             label: '✅ Ready (Uploaded)',count: orders.filter(hasCustomizationBeenReceived).length },
                { key: 'pending',           label: '⏳ Pending Upload',  count: orders.filter(o => !hasCustomizationBeenReceived(o)).length },
                { key: 'approved',          label: '👍 Approved',        count: approvedCount },
                { key: 'sent_to_printer',   label: '🖨️ Printing',        count: sentToPrinterCount + processingCount },
