...
export default function PrinterPortal({ extraItems = [] }: PrinterPortalProps) {
  ...
  const tabs: { id: StatusFilter; label: string; count?: number }[] = [
    ...
    { id: 'pending',     label: '🖨️ Sent to Printer', count: countByStatus('pending')     },
    { id: 'processing',  label: '⚙️ Processing',      count: countByStatus('processing')  },
          ...
          {[
            ...
            <div key={i} className={`glass-panel metric-card${m.variant}`} style={{ borderRadius: isMobile ? 16 : 24, padding: isMobile ? '16px 14px' : '24px', background: '#FFFFFF', borderColor: '#F1F5F9', color: '#1E293B', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
              <div className="flex justify-between align-center mb-4">
                ...
                <span style={{ width: isMobile ? 28 : 36, height: isMobile ? 28 : 36, background: m.variant.includes('accent') ? '#FFF1F2' : m.variant.includes('success') ? '#ECFDF5' : '#F8FAFC', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <i className={`bi ${m.icon}`} style={{ fontSize: isMobile ? 13 : 16, color: m.variant.includes('accent') ? '#FF304C' : m.variant.includes('success') ? '#10b981' : '#64748b' }} />
                </span>
              </div>
              <h3 style={{ fontSize: isMobile ? 28 : 36, fontWeight: 800, color: m.variant.includes('accent') ? '#FF304C' : m.variant.includes('success') ? '#10b981' : '#1E293B', lineHeight: 1, margin: '0' }}>{m.value}</h3>
            </div>
          ))}
        </div>
