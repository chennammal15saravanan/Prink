...
export default function CustomerPortal({
      ...
      if (isLiveMode) {
        const res = await fetch(`/api/orders/${encodeURIComponent(activeOrder.id)}/design`, {
          ...
          body: JSON.stringify({
            ...
            customizationStatus: 'completed',
            images: images.map(img => ({ id: img.id, name: img.name, src: img.src, serverFilename: img.serverFilename || img.name })),
          }),
        });
      ...
      if (res.ok && data.success && data.image) {
        ...
        setImages(prev => {
          ...
          if (updated[0]?.id === data.image.id) {
            setLivePreviewPhoto(data.image.url);
          }
          return updated;
        });
      } else {
  ...
  const customizationBadge = (order: Order) => {
    ...
    if (order.workflowStatus === 'printer_processing' || order.workflowStatus === 'printing' || order.printStatus === 'printing' || order.printStatus === 'processing') return <span className="badge badge-warning" style={{ background: '#fef3c7', color: '#b45309', fontWeight: 700 }}><i className="bi bi-printer-fill" /> Printing in Progress</span>;
