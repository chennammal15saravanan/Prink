  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (order.skuDetails && order.skuDetails.printAreaWidth && order.skuDetails.printAreaHeight) {
        width = order.skuDetails.printAreaWidth * 72;
        height = order.skuDetails.printAreaHeight * 72;
        isLandscape = order.skuDetails.orientation === 'landscape';
      } else {
      ...
      if (order.images && order.images.length > 0) {
        ...
        if (localImgPath && fs.existsSync(localImgPath) && fs.statSync(localImgPath).isFile()) {
