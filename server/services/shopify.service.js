  ...
  for (const item of lineItemsList) {
    ...
    try {
      ...
      if (dbProduct && dbProduct.images && dbProduct.images.length > 0) {
        ...
        // Fallback: Fetch directly from Shopify Admin API if token/shop are configured
        const db = require('../db');
        const settings = await db.getSettings();
        const shop = settings.shopifyStore || process.env.SHOPIFY_STORE || shopifyConfig.store;
        const token = settings.shopifyAccessToken || process.env.SHOPIFY_ACCESS_TOKEN || shopifyConfig.accessToken;
        
        if (shop && token && token !== 'your_access_token_here' && item.product_id) {
          try {
            const pData = await getProductByIdFromShopify(shop, token, item.product_id);
            if (pData) {
              await syncProductToDb(pData);
              if (pData.images && pData.images.length > 0) {
                productImage = pData.images[0].src || pData.images[0];
              }
            }
          } catch (apiErr) {
            console.warn(`Failed to retrieve product details for ${item.product_id} from Shopify API:`, apiErr.message);
          }
        }
      }
    } catch (err) {
