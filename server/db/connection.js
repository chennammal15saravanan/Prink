...
async function connectDB() {
  ...
  const getTargetUri = () => process.process?.env?.USE_MEMORY_DB === 'true'
    ? 'in-memory'
    : (process.env.MONGODB_URI || 'mongodb://localhost:27017/theprink');

  const connectWithUri = async (uri, timeoutMS = 500) => {
